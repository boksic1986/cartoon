...
- 真实视频提交前新增 `prepare-seedance-submit` 离线提交计划；它只生成 JSON，并继续停在真实 Seedance API 调用前。
- Seedance mock 任务生命周期只生成本地 submissions/results、mock request/response 和占位视频文件；它不代表真实视频已经生成。
...
- 新增 `prepare-seedance-submit`，写出 `seedance_submit/submit_plan.json`，要求显式确认外部调用认知和预算上限。
- 提交计划 schema 锁定 `provider=seedance`、`dry_run=true`、`execute_real_requested=false` 和 `STOP_BEFORE_REAL_SEEDANCE_SUBMIT`，并拒绝敏感字符串进入计划 JSON。
- `quality-check` 会在提交计划存在时复验当前 preflight 指纹和费用上限，防止沿用旧提交计划。
- 新增 `submit-seedance-tasks --provider mock` 和 `poll-seedance-tasks --provider mock`，用本地 JSON 账本演练任务提交、轮询和下载路径。
- `quality-check` 会在 task 产物存在时校验 submissions/results schema、提交计划指纹、task id 对齐、mock request/response、占位视频和 `videos/seedance_clips.json`。
- task JSON 和 clips manifest 会进行敏感字符串扫描，防止凭证、敏感请求头或账号标识进入产物。
- 新增 Seedance client contract schema 和 `SeedanceApiClient` 外壳；当前只有本地 `MockSeedanceHttpTransport` 可用，真实网络 transport 默认禁用。
- `submit-seedance-tasks --provider seedance --dry-run --confirm-external-call` 和
  `poll-seedance-tasks --provider seedance --dry-run --confirm-external-call` 只演练本地 mock HTTP
  request/response，不读取 API key，不访问网络，不生成真实视频。
- 新增真实 Seedance transport 与 `--execute-real` 门禁；默认只提交 1 个镜头，API key 仅从环境变量读取。
- 图生视频真实调用要求公网首帧 URL；缺少 URL 时默认阻断，只有显式 `--allow-text-only` 才会进行文生视频试跑。
- 真实轮询下载会写入 `videos/*.seedance_real.mp4`，但 JSON 产物不写 provider 凭证、鉴权请求头或临时远程下载链接。
...
- 批量真实 Seedance 调用和全 10 镜头成本控制策略。
- 真实 Seedance 失败重试调度、断点续轮询和真实计费回填。
- 真实 Seedance 模型名、参数和首帧 URL 的人工复核记录。
- 对 `review/video_motion_review.json` 的逐镜人工审片确认。
- 对 `quality_reports/seedance_cost_estimate.json`、`quality_reports/real_video_preflight.json` 和 `seedance_submit/submit_plan.json` 的人工确认，以及是否允许真实视频调用的明确授权。
- 面向人工审核者的浏览器预览页或表单。
